...
export default function useSurveyStep(config, visitedSteps, i18n, isFromConcierto) {
  ...
  return {
    step: getStep(config, survey, formError, i18n, visitedSteps, isFromConcierto),
    formError,
...
function getStep(config, survey, hasErrors, i18n, visitedSteps, isFromConcierto) {
  if (!config.survey_enabled) {
    return null;
  }
  return {
    ...
    name: (
      ...
      >
        {i18n._(t`${isFromConcierto ? 'Inputs' : 'Survey'}`)}
      </StepName>
    ),
