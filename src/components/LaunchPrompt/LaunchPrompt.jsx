...
function PromptModalForm({ onSubmit, onCancel, i18n, config, resource, isFromConcierto }) {
  ...
  const {
    ...
    visitAllSteps,
    contentError,
  } = useLaunchSteps(config, resource, i18n, isFromConcierto);
  ...
  return (
    <Wizard
      ...
      title={i18n._(t`${isFromConcierto ? resource.name : 'Prompts'}`)}
      steps={
        isReady
          ? steps
          : [
...
function LaunchPrompt({ config, resource = {}, onLaunch, onCancel, i18n, isFromConcierto }) {
  return (
    <Formik
      initialValues={{
