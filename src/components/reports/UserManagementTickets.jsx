...
import { fetchUserReport } from "./../../actions/reports/reportsMain"
...
function UserManagementTickets() {
    ...
    const handleSubmit = (e) => {
        ...
        dispatch(generateToken())
            .then((data) => {
                ...
                dispatch(fetchUserReport(request))
                    .then((res) => {
                        if (res && res.userReport.status !== 200) {
                            const { message } = res.userReport;
                            const text = typeof message === "string" ? message : "Something went wrong. Please try again!";
                            failureToast(text);
                        }
                        const data = res.userReport?.data || [];
                        setUserReport(data);
                    }).catch(() => {
    ...
    const exportToExcel = async (e) => {
        ...
        dispatch(generateToken())
            .then(function (data) {
                var url = reportApiUrls.userExcelReport + "?fromDate=" + excelRequest.startDate + "&toDate=" + excelRequest.endDate +
    ...
    return (
        <>
            <ComponentHeader
                ...
                headerClass=""
                hasShowBreadcrumb
                breadCrumb={[{ name: 'Reports', path: '/reports' }, { name: 'User Management Report', path: '' }]}
            />
            <div className="page">
                <div className="bg-wh" >
                    <form>
                        ...
                        <div className='form-group' id='reports'>
                            <table className="table table-hover my-table">
                                ...
                                <tbody>
                                    {userReport && Array.isArray(userReport.ticketDetails) && userReport.ticketDetails.map((data, index) => {
                                        return (
                                            <tr key={index}>
                                                <td>{index + 1}</td>
                                                <td>
...
export default UserManagementTickets;