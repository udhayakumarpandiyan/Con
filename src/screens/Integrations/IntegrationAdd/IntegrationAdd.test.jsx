...
describe('<IntegrationAdd/>', () => {
  ...
  test('should render properly', async () => {
    ...
    await act(async () => {
      wrapper = mountWithContexts(<IntegrationAdd />);
    });
    expect(wrapper.find('IntegrationAdd').length).toBe(1);
    expect(wrapper.find('IntegrationForm').length).toBe(1);
    expect(IntegrationsAPI.readOptions).toBeCalled();
  });

  test('expect values to be updated and submitted properly', async () => {
    const history = createMemoryHistory({
      initialEntries: ['/integrations/add'],
    });
    IntegrationsAPI.readOptions.mockResolvedValue(options);

    IntegrationsAPI.create.mockResolvedValue({ data: { id: 8 } });
    await act(async () => {
      wrapper = mountWithContexts(<IntegrationAdd />, {
    ...
    expect(IntegrationsAPI.create).toBeCalledWith({
      authorization_grant_type: 'authorization-code',
      client_type: 'confidential',
      description: 'bar',
      name: 'foo',
    ...
    expect(history.location.pathname).toBe('/integrations/8/details');
  });
  ...
  test('should throw error on submit', async () => {
    ...
    IntegrationsAPI.create.mockRejectedValue(error);
    IntegrationsAPI.readOptions.mockResolvedValue(options);
    await act(async () => {
      wrapper = mountWithContexts(<IntegrationAdd />);
    });
  ...
  test('should render content error on failed read options request', async () => {
    IntegrationsAPI.readOptions.mockRejectedValue(
      new Error({
        response: {
          config: {
            method: 'options',
