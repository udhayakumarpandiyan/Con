...
import { TokensAPI, IntegrationsAPI } from '../../../api';
...
const QS_CONFIG = getQSConfig('integrations', {
  page: 1,
  page_size: 20,
  order_by: 'user__username',
});

function IntegrationTokenList({ i18n }) {
  const { id } = useParams();
  ...
  const {
    ...
    useCallback(async () => {
      ...
      const [
        ...
        IntegrationsAPI.readTokens(id, params),
        IntegrationsAPI.readTokenOptions(id),
      ]);
      const modifiedResults = results.map(result => {
        result.summary_fields = {
          user: result.summary_fields.user,
          integration: result.summary_fields.integration,
...
export default withI18n()(IntegrationTokenList);
