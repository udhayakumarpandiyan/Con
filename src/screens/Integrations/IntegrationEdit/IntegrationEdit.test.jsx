...
const integration = {
  id: 1,
  type: 'o_auth2_integration',
  url: '/api/v2/integrations/10/',
  related: {
    named_url: '/api/v2/integrations/Alex++bar/',
    tokens: '/api/v2/integrations/10/tokens/',
    activity_stream: '/api/v2/integrations/10/activity_stream/',
  },
  summary_fields: {
    organization: {
      id: 230,
...
describe('<IntegrationEdit/>', () => {
  ...
  test('should mount properly', async () => {
    ...
    expect(wrapper.find('IntegrationEdit').length).toBe(1);
  });
  test('should cancel form properly', async () => {
    const history = createMemoryHistory({
      initialEntries: ['/integrations/1/edit'],
    });
    ...
    await act(async () => {
      ...
      expect(history.location.pathname).toBe('/integrations/1/details');
    });
  });
  test('should throw error on submit', async () => {
    ...
    IntegrationsAPI.update.mockRejectedValue(error);
    const history = createMemoryHistory({
      initialEntries: ['/integrations/1/edit'],
    });
  ...
  test('expect values to be updated and submitted properly', async () => {
    ...
    expect(IntegrationsAPI.update).toBeCalledWith(1, {
      authorization_grant_type: 'authorization-code',
      client_type: 'confidential',
      description: 'bar',
      name: 'foo',
